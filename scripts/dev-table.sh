...
LOG_FORMAT=json NODE_ENV=development NODE_OPTIONS='--disable-warning=FSTDEP023' tsx src/index.ts \
  ...
  | select(.msg | IN("CUSTOM_ROUTE","CUSTOMER_INPUT","INTENT_CHANGE","AGENT_RESPONSE","CLAUDE_API","INTERRUPT","TOOL_CALL","TOOL_RESULT","TOOL_ERROR","CONVERSATION_ENDED","CR_AGENT_SPEAK_ON","CR_AGENT_SPEAK_OFF","CR_CLIENT_SPEAK_ON","CR_CLIENT_SPEAK_OFF","SILENCE_TIMER"))
  | [
      ...
      (
        ...
        elif .msg == "SILENCE_TIMER"  then (.description // "")
        else "" end
      ) | tostring | gsub("\n"; " ")
    ]
  | @tsv
' | awk -F "\t" '
  BEGIN {
    ...
    colors["CR_AGENT_SPEAK_ON"]  = "\033[90m"   # gray
    colors["CR_AGENT_SPEAK_OFF"] = "\033[90m"   # gray
    colors["CR_CLIENT_SPEAK_ON"] = "\033[90m"   # gray
    colors["CR_CLIENT_SPEAK_OFF"]= "\033[90m"   # gray
    colors["SILENCE_TIMER"]      = "\033[91m"   # bright red
